...
export class ChessBoardComponent implements OnInit, OnDestroy {
  ...
  protected updateBoard(prevX: number, prevY: number, newX: number, newY: number, promotedPiece: FENChar | null): void {
    ...
    try {this.chessBoard.move(prevX, prevY, newX, newY, promotedPiece);}
    catch(e) {
      console.log("Square is not safe exception or Coord not valid");
      localStorage.setItem("is_move_correct", JSON.stringify(false));
      this.chessBoardService.chessBoardState$.next(this.chessBoard.boardAsFEN);
      return;
    }

    localStorage.setItem("is_move_correct", JSON.stringify(true));
    console.log("-------here--------")
